...
do
    ...
    -- PlasmaBeam buffer: "\x03\n\x00\x00\x00PlasmaBeam"
    -- \x03 = type byte, \n = 0x0a = length (10), then "PlasmaBeam" (10 chars)
    local PLASMA_STR = "\x03\x0a\x00\x00\x00PlasmaBeam"
    local function dk_firePlasma()
        if not dk_RemoteEvent then return end
        local buf = buffer.fromstring(PLASMA_STR)
        dk_RemoteEvent:FireServer("UseActorAbility", {buf})
    end
