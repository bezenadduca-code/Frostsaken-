...
tabGlobal:CreateParagraph({ 
    Title = "How it works", 
    Content = "Modifies the Sprinting module's SprintSpeed value directly. Set a custom sprint speed that applies whenever you sprint. Survivor default is 26, Killer default is 27 (varies by killer). Speed is applied in real-time while sprinting." 
})
...
local function speedApply()
    ...
    if not speedHack.originalSpeed then
        speedHack.originalSpeed = m.SprintSpeed or speedGetDefault()
    end
    
    if speedHack.speed > 0 then
    ...
    if char then
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum and speedHack.speed > 0 then
...
tabGlobal:CreateSlider({ 
    ...
    Range = { 1, 100 }, 
    Increment = 1, 
    CurrentValue = speedHack.speed,    
    Flag = "speedValue",    
    Callback = function(v) 
        speedHack.speed = v
        cfg.set("speedValue", v)
        if speedHack.on then speedApply() end
    end 
})
...
tabGlobal:CreateButton({ 
    ...
    Callback = function()
        ...
        Rayfield:Notify({ 
            Title = "Speed Hack", 
            Content = "Sprint speed reset to default (" .. speedGetDefault() .. ")", 
            Image = 4483362458, 
            Duration = 3 
        })
    end
