...
local function jd_forceMaxCharge()
    pcall(function()
        ...
        if janeActor.State then
            janeActor.State.CrystalFullChargeTime = os.clock()
        end
    end)
end
...
-- CRYSTAL INDICATOR TRACKING (Rotates to killer)
...
local jd_cachedIndicator = nil
local jd_cachedEndPoint = nil
local jd_cachedBeam = nil
local jd_cachedAtt0 = nil
local jd_cachedAtt1 = nil

local function jd_findIndicator()
    local char = svc.Players.LocalPlayer.Character
    if not char then return nil end
    
    for _, child in ipairs(char:GetChildren()) do
        if child.Name == "AimIndicator" and child:IsA("Folder") then
            jd_cachedIndicator = child
            jd_cachedEndPoint = nil
            jd_cachedBeam = nil
            jd_cachedAtt0 = nil
            jd_cachedAtt1 = nil
            
            for _, sub in ipairs(child:GetChildren()) do
                if sub.Name == "EndPoint" and sub:IsA("MeshPart") then
                    jd_cachedEndPoint = sub
                elseif sub.Name == "Beam" and sub:IsA("Beam") then
                    jd_cachedBeam = sub
                    jd_cachedAtt0 = sub:FindFirstChild("Attachment0")
                    jd_cachedAtt1 = sub:FindFirstChild("Attachment1")
                end
            end
            return child
        end
...
local function jd_rotateIndicatorToKiller()
    if not jd_cachedEndPoint or not jd_cachedEndPoint.Parent then
        jd_findIndicator()
        if not jd_cachedEndPoint then return end
    end
    ...
    local predPos = killerHRP.Position + vel * 0.3
    
    local direction = (predPos - myPos)
    local distance = direction.Magnitude
    if distance < 0.1 then return end
    
    direction = direction.Unit
    local targetPos = predPos + Vector3.new(0, 0.5, 0)
    
    local currentPos = jd_cachedEndPoint.Position
    local delta = targetPos - currentPos
    local dist = delta.Magnitude
    
    if dist > 0.1 then
        local moveAmount = math.min(jd_trackSpeed * 2.5, 1)
        local newPos = currentPos + delta * moveAmount
        jd_cachedEndPoint.Position = newPos
        jd_cachedEndPoint.CFrame = CFrame.lookAt(newPos, myPos)
    end
    
    if jd_cachedAtt0 and jd_cachedAtt1 then
        jd_cachedAtt0.WorldPosition = myPos + direction * 1.5 + Vector3.new(0, 1.5, 0)
        jd_cachedAtt1.WorldPosition = jd_cachedEndPoint.Position
        
        if jd_cachedBeam then
            local arcAmount = math.min(distance * 0.03, 5)
            jd_cachedBeam.CurveSize0 = arcAmount
            jd_cachedBeam.CurveSize1 = arcAmount * 0.8
        end
    end
    
    if jd_cachedEndPoint then
        local pulse = 0.85 + math.sin(tick() * 4) * 0.15
        jd_cachedEndPoint.Transparency = pulse * 0.3
        jd_cachedEndPoint.Size = Vector3.new(0.008 * (1 + pulse * 0.3), 5 * pulse, 5 * pulse)
    end
end
