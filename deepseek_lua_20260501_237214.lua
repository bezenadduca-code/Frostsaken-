...
local espColors = {
    ...
    survivor  = loadColor("espColSurvivor",  34, 139,  34),  -- Forest Green (dark green)
...
-- FIXED: Using v1prware's simpler approach for items
...
-- FIXED: Use workspace descendants like v1prware for reliability
...
local function espBindWorld()
    ...
    -- FIXED: Item spawns - watch workspace descendants
...
-- FIXED: Items toggle with pcall wrapper
tabVisual:CreateToggle({ Name="Items",      CurrentValue=esp.items,      Flag="espItems",      Callback=function(on) 
    esp.items=on;      
    cfg.set("espItems",on);      
    pcall(function()
        espDoItems(on)
...
tabVisual:CreateColorPicker({
    ...
    Callback = function(col)
        espColors.medkit = col
        espColors.bloxycola = col  -- Update both item colors
        saveColor("espColMedkit", col)
        saveColor("espColBloxy", col)
...
tabVisual:CreateParagraph({
    Title   = "How it works",
    Content = "Tracks c00lkidd's pizza bots (orange), 1x1x1x1 zombies (green), and John Doe's shadow puddles (black disc with red glow). Each uses AncestryChanged to self-clean when the object is removed. Puddles also show a radius indicator that resizes as the puddle grows.",
})
...
do
    ...
    local function chanceGetMotion(hrp)
        ...
        if not chanceMotion[hrp] then chanceMotion[hrp]={lastPos=pos,lastTime=now,velocity=Vector3.zero,accel=Vector3.zero}; return Vector3.zero,Vector3.zero end
        local data=chanceMotion[hrp]; local dt=now-data.lastTime; if dt<=0 then return data.velocity,data.accel end
    ...
    local function chanceGetPing()
        ...
        return (ok and stat or 100)/1000
    end
    ...
    local function chancePredict(hrp)
        ...
        local ping=chanceGetPing(); local lead
        if chancePredMode=="Velocity" then lead=chancePredValue
        elseif chancePredMode=="Ping" then lead=ping*chancePredValue
    ...
    lp.CharacterAdded:Connect(function(c)
        local hum=c:WaitForChild("Humanoid"); local anim=hum:WaitForChild("Animator")
